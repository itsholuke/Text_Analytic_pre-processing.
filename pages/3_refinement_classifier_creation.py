# ───────────────────────────────────────────────────────────
#  refinement_classifier_creation.py  (numeric‑flag, final) 
#  ----------------------------------------------------------
#  • Single raw‑caption CSV ➜ auto dictionary ➜ classify ➜
#    optional ground‑truth (upload / manual) ➜ metrics ➜ download
#  • Session flags stop the jump‑back; bug‑fixed variable names
# ───────────────────────────────────────────────────────────
import ast, re
import pandas as pd
...
if st.button("🔹 2. Compute Metrics", disabled=not st.session_state.pred_ready):
    ...
    if not st.session_state.gt_df.empty:
        ...
        if flag_col in gt.columns:
            ...
            st.error("Ground‑truth CSV must have 'true_label' or '{flag_col}'.")
            st.stop()
