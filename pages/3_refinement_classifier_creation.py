...
#  (numeric-flag, final consolidated)
# ───────────────────────────────────────────────────────────
import ast, re
import pandas as pd
...
if st.button("🧠 Step 4 — Generate / refine dictionary"):
    ...
    pos_df = df_temp[df_temp["cleaned"].apply(lambda x: any(tok in x.split() for tok in seeds))]
    ...
    if pos_df.empty:
        ...
        freq = pos_df["cleaned"].str.split(expand=True).stack().value_counts()
        ctx_terms = [w for w in freq.index if w not in stop_words and w not in seeds][:30]
